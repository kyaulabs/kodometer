...
4. Create the PR with `gh-bot`. Features/fixes/docs target `develop`; release PRs target `main`; release back-merges target `develop`. Use `.github/PULL_REQUEST_TEMPLATE.md` and include an executable, concrete Test Plan. Distinguish performed checks from checks still pending.
