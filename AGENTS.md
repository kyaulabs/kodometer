...
- Wallet writes/removals are immediate and shared. Profile/account selection belongs in settings, not the popup; selections/preferences follow Apply/Cancel/Defaults. Preserve controller bindings and selected-only context invalidation. The internal runtime-selection helper remains tested but is not exposed by popup controls.
...
- Observed subscription-quota history may persist for up to 30 days in bounded, owner-only local storage. Record only successful current-context refreshes, partition by the selected profile/account and pinned authenticated identity, and never serialize credentials, private selectors, labels, or credential paths. Restore only after a matching successful refresh; preserve missing observations and resets in storage instead of inventing activity. Charts carry the last observed percentage horizontally through gaps and to the view's end, with continuous fill after the first known sample. Drops, resets, and quota increases are vertical steps at the next observation, not gaps or invented samples. Hover shows a time-position marker and the last observed percentage at that position.
- Donut mode is provider-specific: one meter per quota provider, with differently colored nested rings for its enabled windows and provider-only tooltips. Window switches apply across quota displays, independently of notification policy. Hide Codex Spark by default with one master toggle for all Spark windows; do not expose duplicate per-window Spark switches or apply their obsolete hidden-window entries. Popup bars default to remaining quota with a used-quota option. Keep the popup width stable and fit its height to the selected content within screen bounds. Over-tall content uses inner page tabs, not vertical scrolling. Keep provider hover backgrounds padded around all content; put provider links below usage/history and retain only the footer's confirmed history-clear icon. Per-provider settings colors are the sole source for tabs, bars, and chart accents; legacy Session/Weekly donut overrides are no longer applied. Default Codex to #A28BE0, Kimi Code to #49A3B0, and OpenRouter to #C8FF00 without overwriting saved provider overrides. Use visually balanced footer icons (settings is two logical pixels smaller than the other action icons), symmetric top/bottom provider-tab padding, and the logo's Cloud/Ink color for Overview on dark/light surfaces.
