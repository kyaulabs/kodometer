...
The popup keeps a stable width and fits its height to the current content, rather than retaining a taller tab's size. Provider rows include padding inside their hover background. Footer actions use matching icon sizes, including a symbolic settings icon. The Overview tab uses the logo's Cloud (`#F7F5FB`) accent on dark surfaces and Ink (`#252333`) on light surfaces. When a tab exceeds the available screen-height budget, numbered page tabs appear inside it. Pages break between sections where possible; an exceptionally tall section continues on another page. Keyboard focus follows the active page, and popup pages have no vertical scrollbar. Overview includes all enabled providers with data. Settings dialogs can still scroll.
...
The chart connects observations with straight lines, including intervals longer than 15 minutes; unchanged percentages form horizontal lines. Reported resets and increases in remaining quota still start a new line. Connecting lines are visual interpolation, not additional recorded samples. A single observation remains a point, and missing intervals are not zero. The chart retains the latest actual observation in each five-minute bucket, so faster polling does not create unlimited samples.
